...
public class SchemaBuilder {
    ...
    private final Set<Name> singletons = new HashSet<>();
    ...
    public SchemaBuilder updateSymbolicName(String name) {
    ...
    public class ObjectTypeBuilder {
        ...
        public ObjectTypeBuilder singleton() {
            singletons.add(typeName);
            return this;
        }
        ...
        public class FieldBuilder {
            ...
            public FieldBuilder multiplicityOptional() {
                return multiplicity(Multiplicity.of(0, 1));
            }

            public FieldBuilder multiplicityRequired() {
                return multiplicity(Multiplicity.of(1, 1));
            }


            public FieldBuilder multiplicityCollection() {
                return multiplicity(Multiplicity.of(0, -1));
            }
        ...
        public ObjectTypeBuilder makeAbstract() {
            SchemaBuilder.this.abstractTypeNodes.add(typeName);
            return this;
        }
        ...
        public SchemaBuilder endObjectType() {
    ...
    public class ActionBuilder {
        ...
        public ActionBuilder inputArg(String name, String targetName) {
            this.buildInputArg(name, targetName).endArgument();
            return this;
        }
        ...
        public ActionArgBuilder buildReturnArg(String resultType) {
            return this.buildReturnArg(Name.identifier(resultType));
        }

        public ActionArgBuilder buildReturnArg(Name resultType) {
            return this.buildOutputArg(SpecialNames.RESULT_SUCCESS.prefixWith(actionName), resultType);
        }

        public ActionArgBuilder buildOutputArg(Name argName, Name typeName) {
            graphBuilders.edge(actionName, argName, typeName);
            actionOutputs.add(Triple.edge(actionName, argName, typeName));
            return new ActionArgBuilder(argName, typeName);
        }
    ...
    public class ActionGroupBuilder {
        ...
        public ActionBuilder action(String actionName) {
            ...
            Name lbl = id.elementOf(actionGroupName);
            Triple edge = Triple.edge(actionGroupName, lbl, id);
            graphBuilders.edge(actionGroupName, lbl, id);
            actionGroupMemberships.add(edge);
            actions.add(id);
    ...
    public SchemaBuilder(Name symbolicName, Universe universe) {
        this.graphBuilders = new GraphBuilders(universe, true, false);
        this.symbolicName = symbolicName;
    }

    public SchemaBuilder stringValueType(String name) {
    ...
    public SchemaBuilder floatValueType(String name) {
        graphBuilders.node(Name.identifier(name));
    ...
    public SchemaBuilder intValueType(String name) {
        graphBuilders.node(Name.identifier(name));
    ...
    public SchemaBuilder boolValueType(String name) {
        graphBuilders.node(Name.identifier(name));
        this.valueTypeNodes.put(Name.identifier(name), BoolDT.getInstance());
        return this;
    }

    public SchemaBuilder enumValueType(String name, Class<? extends Enum<?>> enumClass) {
        List<Name> literals = new ArrayList<>();
        for (Enum<?> e : enumClass.getEnumConstants()) {
            literals.add(Name.identifier(e.name()));
        }
        return this.enumValueType(name, literals);
    }

    public SchemaBuilder enumValueType(String name, List<Name> literals) {
        graphBuilders.node(Name.identifier(name));
        this.valueTypeNodes.put(Name.identifier(name), EnumValue.getInstance(literals));
        return this;
    }


    public SchemaBuilder customValueType(String name, DataTypePredicate dataTypePredicate) {
        graphBuilders.node(Name.identifier(name));
    ...
    Sketch buildUnderlyingSketch() {
        ...
        for (Name singleton : singletons) {
            graphBuilders.startDiagram(Singleton.getInstance());
            graphBuilders.map(Universe.ONE_NODE_THE_NODE, singleton);
            graphBuilders.endDiagram(Name.anonymousIdentifier());
        }
    ...
    public Schema build() {
        return new Schema(buildUnderlyingSketch(), this.customName);
    }
