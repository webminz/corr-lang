...
corrfile: WHITESPACE? (fileImport)* (endpointDefinition | corrspec | consistencyRule)+ EOF;
...
commonality: commonalityType WHITESPACE?
            ...
            (WHITESPACE 'as' WHITESPACE commonalityName)?
            (subCummonalities)?
            (WHITESPACE 'when' WHITESPACE keyExpression)?
            (WHITESPACE 'check' WHITESPACE (consistencyRuleBody || consistencyRuleRef))?
            ';';

commonalityType: 'relate' | 'sync' | 'identify';
commonalityName: IDENTIFIER;
consistencyRuleRef: IDENTIFIER;
subCummonalities: 'with' '{' commonality+ '}';
...
keyExpression: keyAlternative ('||' keyAlternative)*;
keyAlternative: keyLiteral ('&&' keyLiteral)*;
keyLiteral: keyArgument  (keyIdentity | keyRelation);
keyIdentity: '==' keyArgument ('==' keyArgument)*;
keyRelation: '<~>' keyArgument ('==' keyArgument)*;

keyArgument: (elmentRef | constant) (keyArgumentSuffix)*;
keyArgumentSuffix: '++' (elmentRef | constant);


constant: stringConstant | intConstant | floatConstant | boolConstant | enumConstant;

stringConstant: STRING;
intConstant: INTEGER;
floatConstant: FLOAT;
boolConstant: BOOL;
enumConstant: 'literal' STRING;
