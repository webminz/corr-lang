...
public class FileGoal extends LanguageGoal {
    ...
    private ComprData getComprDataInstance(CorrSpec correspondence) throws IOException, UnsupportedFeatureException, TechSpaceException, URISyntaxException {
        ...
        if (isWhitebox) {
            ...
            if (queryFile == null) {
                throw new UnsupportedFeatureException("There is at least one io.corrlang.components.server endpoint, thus you have to specify a Query against whose results the verification should be applied!");
            } else {
                return null;
